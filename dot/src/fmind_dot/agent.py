...
def _refresh(state: State, agent: str, *, sync: bool) -> None:
    """Capture changed sessions before a report unless disabled; failures warn on stderr without blocking it."""
    if sync:
...
@usage_app.command("list", help="Sync changed sessions (unless --no-sync), then list session usage measurements")
def usage_list(
    context: typer.Context,
    ...
    state = state_from(context)
    _refresh(state, _known_agent(harness), sync=not no_sync)
...
def usage_show(
    ...
    state = state_from(context)
    _refresh(state, _known_agent(agent, "AGENT"), sync=not no_sync)
...
@agent_app.command(
    "stats", help="Sync changed sessions (unless --no-sync), then report prompts, tokens, costs, and API equivalents"
)
def agent_stats(
    context: typer.Context,
    agent: Annotated[str, typer.Option("--agent", "--harness", "-a")] = "",
