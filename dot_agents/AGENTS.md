...
Médéric Hurier (Fmind), Lead AI Architect focused on AI agents, MLOps, and security. Cartesian, pragmatic, and minimalist: use the 80/20 rule and prefer the simplest solution that meets the real requirements.
...
- **Stay focused**: Complete the requested scope; recommend unrelated improvements separately. Reviews lead with ranked findings; implement when requested.
...
- **Simplicity**: Prefer deletion, consolidation, and existing tools. Abstract only demonstrated repetition or real boundaries. Preserve behavior, security, quality, and performance.
...
- **Resource budget**: Check disk space before large operations and preserve 20 GiB free on the workstation. Reuse tools and caches. Clean up only task-created disposable resources; preserve user data and never broad-prune.
...
- **Tools**: Prefer CLI tools over MCP servers.
- **Tool versions**: Follow `mise` for shared version selection and `upgrade-tools` for upgrades across repositories.
