...
- **Visual defaults**: For my projects, use Google Sans for general text and Google Sans Code for code, with GoogleSansCode Nerd Font Mono in terminals. Use [fmind/theme](https://github.com/fmind/theme) as the default theme unless the project explicitly specifies otherwise.
...
- **Resource budget**: Before large downloads, builds, model pulls, or disposable checkouts, check free space on the affected filesystem; preserve at least 20 GiB on the 100 GiB workstation. Reuse compatible shared tool versions and caches. At task end, including failure, remove only task-created disposable containers, volumes, images, builders, environments, and scratch artifacts after checking ownership and consumers; preserve requested outputs and user data. Report retained resources and why. Never use broad pruning to clean up one task. `containerize` owns Docker teardown; `mise` and `upgrade-tools` own version reuse.
...
- **Tool versions**: `fmind/dot` tracks `latest` by default for every mise tool, including Python; its managed mise lockfile supplies the shared exact versions. Other repositories under `~/fmind`, `~/fmind-ai`, and `~/mlops-courses` use fixed mise versions by default. `mise` owns baseline selection; `upgrade-tools` also checks those repositories and propagates shared versions with per-repository validation and documented exceptions.
