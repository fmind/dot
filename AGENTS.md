...
- **Theme**: [fmind/theme](https://github.com/fmind/theme) owns the palette and native app files. Use dark syntax on white: charcoal variables, blue keywords/calls, green strings, orange literals/warnings, red errors, grey comments, purple types/builtins and teal information. Bright fills use contrasting labels. Keep upright GoogleSansCode Nerd Font Mono at 14pt in Ghostty and `bold-is-bright = false`.
  ...
  - `.chezmoiexternal.toml.tmpl` uses `theme_variant` and `theme_ref`, with `theme_local_path` taking precedence for an unpublished local checkout. An invalid configured local checkout fails; it never silently selects the older release. Run `chezmoi init` when adding or changing host theme data.
  - Prefer native remote references or chezmoi externals for standalone theme files. When a tool requires merging styles, copy only its theme block and add a comment linking to the exact upstream file. Keep unpublished standalone themes copied here until a release provides them; then move them to externals.
  - Copy the native Starship, gh-dash, bottom, Lazydocker, LazyGit and Fastfetch blocks and bat, Yazi, Atuin and lsd theme files into their managed sources. gh-dash uses nested text/background/border mappings. Lazygit uses explicit light selection surfaces. Keep Zellij pane frames enabled so top/bottom bars and panes remain distinct. Fish selects the external k9s skin with `K9S_SKIN=theme`.
  - bat and delta share the native `fmind.tmTheme`; `run_onchange_after_bat-theme.sh.tmpl` rebuilds its derived cache after installation. Terminal-native tools use the matching ANSI palette; do not remap the xterm cube or greyscale ramp. Ghostty sets `minimum-contrast = 1` to preserve the exact selected colours.
...
- `.chezmoiexternal.toml.tmpl` fetches the theme files from `fmind/theme` during apply, while native themes that must be merged or installed with derived assets are copied into managed sources.
