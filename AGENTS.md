...
- **Dependency locks**: Keep the global mise lockfile at format 1 for version-only npm/pipx installs; do not upgrade its format. The repository lockfile may use format 2 for native tools. Tool versions are pinned, but their transitive dependencies may change on reinstall. Preserve the Colab and pgcli `with` requirements. `mise run lock` captures the shared global lockfile. Python application dependencies remain locked in `dot/uv.lock`.
