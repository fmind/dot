...
- **Dependency locks**: Keep the global mise lockfile at format 1 for version-only npm/pipx installs; do not upgrade its format. The repository lockfile may use format 2 for native tools. Tool versions are pinned, but their transitive dependencies may change on reinstall. Preserve the Colab and pgcli `with` requirements. `mise run lock` captures only the shared global lockfile; optional locks stay host-local. Python application dependencies remain locked in `dot/uv.lock`.
...
- **Add tool**: Append to `dot_config/mise/config.toml.tmpl` → `chezmoi apply --force ~/.config/mise` → `mise run lock` → `mise run tools`.
