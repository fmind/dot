...
1. Keep the key in encrypted `secrets.fish` and use a runtime environment reference. Never put it in a model configuration, command argument, diagnostic output, or skill example.
...
OpenCode configuration belongs to [agent-harnesses](../../agent-harnesses/references/opencode/GUIDE.md). Keep a small provider block using `options.apiKey: "{env:OPENROUTER_API_KEY}"`; configure `model` and `small_model` explicitly to avoid auxiliary calls through a different provider. Shared skill discovery, compaction, permissions, and built-in agents need no OpenRouter plugin.
