...
1. Obtain an explicitly supplied Developer API credential for the authorized integration. This workstation no longer stores a personal `GEMINI_API_KEY`; its `VERTEX_API_KEY` belongs to GCP Agent Platform and must not be sent to AI Studio. Keep Developer API credentials in a process-scoped, application-specific variable or secret store, not globally exported as `GEMINI_API_KEY` or `GOOGLE_API_KEY`.
1. Inspect the installed `google-genai` client signature. Set `enterprise=False` (older SDKs: `vertexai=False`) and pass the intended key explicitly so ambient Google keys or backend flags cannot redirect the client. Keep backend selection scoped to the application instead of changing the whole shell.
