...
1. Read `GEMINI_API_KEY` from the environment. On this workstation use `dot secret run GEMINI_API_KEY -- uv run app.py` to load the personal key for that process only; use ordinary `uv run` with explicit customer credentials or ADC for customer work. Google SDK auto-discovery gives `GOOGLE_API_KEY` precedence when both variables exist; pass the intended key explicitly to avoid selecting an unrelated credential.
