...
Write the specification contract from [briefs](briefs.md). For a compact, low-risk change, combine or omit immaterial sections. Always retain the decision summary, goals and non-goals, identified requirements, acceptance and edge cases, relevant data and trust boundaries, and assumptions or open decisions; never omit a section because it exposes unresolved risk. After approval, hand off to implementation-plan, or to [github-issues](../../github-issues/SKILL.md) only when the user asks for issue drafts.
