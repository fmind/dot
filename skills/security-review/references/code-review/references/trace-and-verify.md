...
For an authorized fix, add a regression at the public boundary and prove it fails before the fix. Re-test the fix against the reproducer, and verify legitimate operations still succeed. Prefer correcting the shared enforcing boundary over adding equivalent checks independently to every caller.
