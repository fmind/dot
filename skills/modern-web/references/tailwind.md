...
Use the upstream standalone `tailwindcss` executable through the project's locked mise toolchain. Preserve an existing asset build; a new Python web project does not need a Node package graph just to compile CSS. [modern-web](../SKILL.md) owns this workflow; Django or Litestar owns serving and collecting the output.
