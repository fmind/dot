...
- **Upstream links**: when a skill teaches a versioned tool, library, or service, its `Documentation` section carries a `Releases:` bullet naming that subject's release page or changelog next to its documentation link; harness skills carry the same pair in `Top Links`. Link the subject the skill teaches, not every tool it invokes — the dedicated skill owns those. Practice skills with no upstream release stream keep documentation and provenance only. Check each URL resolves before committing it.
