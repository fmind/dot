...
- Design examples for bounded output: select paths, fields, time windows, and result limits before fetching; summarize large saved artifacts locally. Preserve pagination/completeness markers, warnings, failure locations, and exit status. JSON is for structure, not automatically brevity. Use native quiet flags only after checking their semantics; never reduce verification scope or discard diagnostics to save tokens. Put tool-specific recipes in their existing owner and use [mise output conventions](../../mise/references/task-conventions.md#concise-output) for task templates.
