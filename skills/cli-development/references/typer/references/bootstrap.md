...
1. Add the installed command to `pyproject.toml`:
   ...
   <slug> = "<package>:main"
   ```
1. Use [test_smoke.py](../templates/test_smoke.py) and [test_cli.py](../templates/test_cli.py) in `tests/`, replacing the library's version-only example. They exercise help, version, usage errors, streams, and Bash/Zsh/Fish completion generation. The Bash generation test accepts Typer's warning when the detected Bash is older than 4.4, including macOS's bundled Bash 3.2; generating a script does not establish that it works in that shell. Set `mise`'s `watch` command to `uv run <slug> greet` when useful; otherwise retain the shared test watcher.
