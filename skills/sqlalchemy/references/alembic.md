...
Use this guide for Alembic mechanics; [data-migration](../../data-migration/SKILL.md) owns compatibility, backfills, execution authority, and recovery. Inspect the installed Alembic/SQLAlchemy versions, dialect, migration configuration, and revision history before editing.

1. Reuse the repository's migration task and configuration. If Alembic is missing, add it with `uv add alembic` when the deployed migration command needs it, or to the project's development group for development-only use. Initialize only a new migration directory; choose `uv run alembic init migrations` or `uv run alembic init -t async migrations` according to the driver.
