...
1. **Diagnose selectively**: `dot doctor --json` is local; `dot doctor --deep --json` also probes authentication. `dot agent doctor --agent codex` checks one agent's user-level notify hook events, command types, explicit Claude/Codex disable switches, last sync, and archive readability. This static check does not prove project/policy overrides, Codex hook trust, or desktop delivery; use the harness's native hook inspection for effective session behavior. Diagnostics share the `dot.diagnostics/v1` envelope with scope, passed, and checks.
