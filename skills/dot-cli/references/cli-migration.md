...
The two old report routes remain hidden deprecated aliases. They print migration guidance to stderr and return the canonical `dot.agent.stats/v2` report shape. Session storage health remains under `dot agent session stats`; usage record inspection remains under `dot agent usage list` and `show`.
