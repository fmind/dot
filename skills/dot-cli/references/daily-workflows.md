...
The active `sessions/v3` store keeps one bundle per agent session: a manifest line with usage, then LF-only JSONL records. Copilot reads transcript and usage in one database read transaction, and its source signature includes the write-ahead log. Grok's signature covers both transcript and signals, including signals-only sessions. The store reads parser 3, 4, and 5 bundles; parsers 3 and 4 come from stores migrated from v2 and show status `legacy` until sync recaptures an available source. Dot 7.0.4 was the last release to migrate `sessions/v2`; a later release refuses a v2-only store without modifying it.
