...
The active `sessions/v3` store keeps one private bundle per agent session, `<agent>/<session_id>.jsonl`: a manifest line (schema 3) with provenance, counts, source signature, and usage, then the normalized records. Sync serializes writers with an owner-only `.write.lock` and replaces the whole file atomically, only when the new parse has at least as many records and its usage extraction succeeded; otherwise it keeps the archived copy and reports the failure. Sync writes parser 5 and reads parser 3, 4, and 5 bundles; parsers 3 and 4 come from stores migrated from v2 (through Dot 7.0.4) and are flagged as legacy until sync recaptures their sources. A store that exists only as `sessions/v2` fails with the migrating release to use, and is never modified; earlier stores and standalone usage files are never read. Unsupported formats fail queries and sync before any change.
