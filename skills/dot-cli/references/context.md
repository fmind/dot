...
Combined global/local skill discovery must be below 3,500 estimated tokens; each scope’s AGENTS.md plus discovery must independently be below 5,000. Exactly the limit fails. `--check` exits 1 on any failure. The `dot.agent.context/v3` JSON envelope adds `budgets.discovery` to v2’s global/local budgets, with `limit_exclusive`, `estimated_tokens`, `remaining`, and `passed`; consumers should check the version and overall `passed`.
