...
1. **Health when diagnosing**: use `dot verify` for the workstation or `dot agent doctor --json` for fast integration metadata; `dot agent doctor --deep --json` adds source hashing and full archive validation. Ordinary session queries do not require a workstation audit.
...
For bounded metadata retrieval, use `dot agent session list --limit 10 --json`, then `dot agent session show <identity>`. `show --content` explicitly includes private prompts and responses; use it only when their contents are needed. Prefer the CLI query contract before opening its derived SQLite store; [agent-usage](../agent-usage/SKILL.md) owns deeper usage analysis.
