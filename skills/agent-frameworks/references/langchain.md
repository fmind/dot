...
1. **Attach MCP tools** with `uv add "langchain[mcp]"` and `MCPAdapter`, building the agent inside `async with` so the tools keep their client. [mcp-setup](../../mcp-setup/SKILL.md) owns host configuration and [MCP implementation](../../agent-protocols/references/mcp/GUIDE.md) owns servers you publish.
