---
name: langgraph
description: "LangGraph state, persistence, streaming, interrupts, and durable execution."
---

# LangGraph

Own the topology when the workflow is more than "loop until done": classification before routing, parallel fan-out, or deterministic steps between model calls. [langchain](langchain.md)'s `create_agent` already returns a compiled graph that drops into a `StateGraph` as a node with its middleware intact, so reach here for the surrounding structure, the persistence contract, and resumable execution.
...
1. **Set durability per run, not at compile**: pass `durability="sync"` for high-value writes, `"async"` for the default trade-off, and `"exit"` only when losing a mid-run crash is acceptable — `"exit"` persists on successful exit, an error, or an interrupt, but cannot recover intermediate state after a process crash.
...
1. **Choose the streaming contract**: `stream`/`astream` with `stream_mode` values (`updates`, `values`, `messages`, `custom`, `checkpoints`, `tasks`, `debug`) serves stable consumers. The typed `stream_events(..., version="v3")` API is experimental in 1.2.11 and emits `LangChainBetaWarning`; adopt it deliberately against the locked release and test its event shape. Emit progress from inside nodes with `get_stream_writer()`.
...
Upstream: [langchain-ai/langchain-skills](https://github.com/langchain-ai/langchain-skills). Use `ecosystem-primer` for the package boundaries, then `langgraph-fundamentals`, `langgraph-persistence`, and `langgraph-human-in-the-loop`; take `langgraph-cli` only for that deployment workflow. The repository keeps its skills under `config/skills/`, includes an MIT license, and offers `skills` CLI, `install.sh`, and Claude plugin installation. Apply the shared [vendor-skill policy](../../agent-project/references/vendor-skills.md) to the selected immutable snapshot before a project-scoped installation; its private npm manifest is development tooling, not a published skill package.
...
- Companion skills: [langchain](langchain.md) (agent loop and models), [python-async](../../python-stack/references/python-async/GUIDE.md) (cancellation and shutdown), [observability](../../observability/SKILL.md) (traces), [agent-evaluation](../../agent-evaluation/SKILL.md) (quality comparisons).
