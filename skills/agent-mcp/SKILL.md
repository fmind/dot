---
name: agent-mcp
description: Configure MCP servers for coding agent hosts with each host's native add command at project or user scope. Use when an agent needs an MCP server.
license: MIT
metadata:
...
1. **Identify host and scope**: inspect the installed `agy`, `claude`, `codex`, `copilot`, `cursor-agent`, `grok`, or `opencode` interface and choose project versus user configuration.
