...
The `.local.toml` suffix makes mise write optional tools into `~/.config/mise/mise.local.lock`, rather than the shared `mise.lock`. `mise lock --global` updates both; `mise run lock` captures only the core lockfile with `chezmoi re-add`. The global lockfile uses format 1: npm and pipx versions are pinned, while their dependencies resolve during installation. The repository lockfile uses format 2 for native tools without dependency files. Preserve the global format; do not use `mise lock --global --upgrade`. Mise 2026.9.10 prints an advisory format-upgrade warning during global lock refresh; keep it visible rather than upgrading away from the chosen strategy. Keep optional lockfiles and any generated dependency files host-local. Never copy a project task file into this global fragment directory.
