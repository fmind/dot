...
1. **Verify the immutable result**: scan the recorded digest, generate a CycloneDX SBOM, sign it, pin the expected certificate identity and issuer during verification, and attest the SBOM per [trivy](../../../security-review/references/trivy/GUIDE.md) and [cosign](../cosign.md). For a multi-platform index, scan every child and bind each SBOM to that child's digest before promoting the index; the [CD template](../../../github-actions/references/ci-cd/templates/cd.yml) implements this sequence.
