...
For a new application, create the minimal foundation through [python-stack](../../../../python-stack/references/foundation/GUIDE.md), then apply only the integrations the application needs before final qualification. Existing services retain their framework, server, and database choices.
...
1. Replace the foundation example with [init.py](../templates/init.py) at `src/<package>/__init__.py`. Add `src/<package>/__main__.py`:
   ```python
   from . import main
...
1. Copy [env.example](../templates/env.example) to `.env.example` and the ignored `.env` before importing the application. Require `DATABASE_URL`, keep CORS closed unless explicitly configured, and use fake local values for offline qualification. Add `[env]` with `_.file = ".env"` to `mise.toml`; [mise](../../../../mise/SKILL.md) owns dotenv loading.
...
1. Replace the library example tests with [test_web.py](../templates/test_web.py) and [test_integration.py](../templates/test_integration.py); copy [conftest.py](../templates/conftest.py) to the project root. Its Postgres fixture starts Docker only when requested by the integration test. Add `"conftest.py" = ["S105"]` to `[tool.ruff.lint.per-file-ignores]` for deliberate fake fixture secrets when needed.
...
- Keep `/health` independent of external services. The database example's `/ready` gives pool acquisition, connection establishment, and `SELECT 1` a shared five-second deadline, returns 503 on database or connection failure, and preserves external cancellation. Adapt that deadline to the deployment probe's budget; cancellation cleanup can extend elapsed time. Offline request tests use a reserved, non-listening loopback port instead of external DNS.
