...
1. **Add project files**: [mise.toml](mise.toml), [lefthook.yml](lefthook.yml), [gitignore](gitignore) as `.gitignore`, and [AGENTS.md](AGENTS.md); configure dprint through [dprint](../../dprint/SKILL.md). Add the exact Python mise pin, replace every scaffold tool selector with its selected exact version, and align `.python-version` before installing. Fill the README and license through [repository-docs](../../repository-docs/SKILL.md) and [project-license](../../project-license/SKILL.md) before building.
1. **Add the library example** as `src/<package>/__init__.py` from [init-library.py](init-library.py) and `tests/test_library.py` from [test_library.py](test_library.py). Replace the generated `hello()` placeholder and keep `src/<package>/py.typed`; a library needs no `__main__.py` or `[project.scripts]`.
