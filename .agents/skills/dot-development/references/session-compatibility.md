...
Manifest schema 2 includes `usage.json` in validation, publication, and compaction; `SESSION_PARSER_VERSION` and `READABLE_PARSER_VERSIONS` in `dot/src/fmind_dot/archive/store.py` own the current and readable parser versions, documented in [contracts](../../../../skills/dot-cli/references/contracts.md). Never update usage separately from the transcript. The active store is `sessions/v2`. Parser 4 introduced deduplicated Claude streaming usage and reconciled timestamped request samples for Claude/Codex. Older admitted bundles remain readable for recovery, with legacy accounting flagged; usage selects the highest admitted parser, then latest ingestion per session. Recapture creates a current-parser generation only from available sources and never rewrites older generations. Parser 1/2, schema 1, and standalone usage remain unsupported. Compaction keeps parser generations separate. Provider-source pruning is outside Dot.
