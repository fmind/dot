...
The active store is `sessions/v3`: each `<agent>/<session_id>.jsonl` holds a manifest line (schema 3, including `usage` and the source signature) followed by normalized records. `SESSION_PARSER_VERSION` and `READABLE_PARSER_VERSIONS` in `store.py` own the current and readable parser versions, documented in [contracts](../../../../skills/dot-cli/references/contracts.md). Parsers 3 and 4 remain from stores migrated from v2 and are flagged as legacy accounting until sync recaptures their sources. The v2 migration shipped through Dot 7.0.4; a store that exists only as `sessions/v2` now fails closed with that release named, and `sessions/v2` stays untouched for the owner to remove. Provider-source pruning is outside Dot.
