...
`dot secret run` preserves an existing environment value; an empty value fails instead of selecting the personal key. It loads only the named credential and preserves child arguments, terminal I/O, and exit status. The scoped keys are `ANTIGRAVITY_SDK_API_KEY`, `VERTEX_API_KEY`, `JULES_API_KEY`, `STITCH_ACCESS_TOKEN`, and `OPENROUTER_MANAGEMENT_KEY` (read-only usage reporting for the private brain's `openrouter-activity` sensor). Use ordinary commands for customer profiles and ADC. Personal model integrations default to GCP Agent Platform with ADC, project `ai-studio-fmind`, location `global`, model `gemini-3.8-flash`, and high thinking; OpenCode keeps OpenRouter. The GCP Agent Platform key replaces the personal AI Studio key and is explicit-only or a last resort after reporting ADC failure. Never export it as `GEMINI_API_KEY` or `GOOGLE_API_KEY`. See [model defaults and billing](skills/model-providers/references/gcp-agent-platform.md) and [scoped credentials](skills/dot-cli/references/authentication.md#scoped-credentials).
